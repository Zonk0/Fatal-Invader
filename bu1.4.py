...
class Bullet(pygame.sprite.Sprite):
    def __init__(self,x,y):
        ...
        if player.buff>=4:
            self.image=random.choice(rainbow_bullet)
        self.mask = pygame.mask.from_surface(self.image)
        self.rect=self.image.get_rect()
        self.rect.bottom=y
        self.rect.centerx=x
...
def healthbar(surf,x,y,health):
        if health<0:
            health=0
        Length=WIDTH-50
        Height=15
        fill=(health/500)*Length
        outline=pygame.Rect(x,y,Length,Height)
        fill=pygame.Rect(x,y,fill,Height)
        pygame.draw.rect(surf,RED,fill)
        pygame.draw.rect(surf, BBLUE,outline,2)
...
def menu():
    ...
    while in_menu:
        screen.fill(BLACK)
        screen.blit(menubg_img,(WIDTH-650,HEIGHT/HEIGHT))  
        
        draw_text(screen, 'Fatal Invader', 70, WIDTH/2, HEIGHT/4.5,BBLUE)
        draw_text(screen, 'Fatal Invader', 70, WIDTH/1.95, HEIGHT/4.4,MAGENTA)
        draw_text(screen, 'Ver 1.4', 20, WIDTH/1.1, HEIGHT/60,WHITE)
        ...
        if selected == 'Begin':
            ...
            draw_text(screen, 'Begin', 45, WIDTH/2.05, HEIGHT/1.77,RED)
        else:
            draw_text(screen, 'Scores', 40, WIDTH/2.03, HEIGHT/1.46,MAGENTA)
            draw_text(screen,'>', 40, WIDTH/3, HEIGHT/1.45,WHITE)

        if score:
           draw_text(screen, 'Scoreboard', 40, WIDTH/2, HEIGHT/2.6,BBLUE)
           draw_text(screen, 'Name:Level:Score', 25, WIDTH/2, HEIGHT/2.3,MAGENTA)
           pos = WIDTH/4
           index = 1
           score_y = 0
           ...
           for name, level, score in top3:
                ...
                draw_text(screen, score_str, 20, pos * index - score_len , HEIGHT/2.05 + score_y ,WHITE)
                index += 1
                score_y += 30
                
        draw_text(screen, 'Begin', 45, WIDTH/2, HEIGHT/1.75,BBLUE)
        draw_text(screen, 'Scores', 40, WIDTH/2, HEIGHT/1.45,WHITE)
...
def pause():
    paused=True
    if paused==True:
        draw_text(screen, 'Pause',70,WIDTH/2,HEIGHT/2.5,BBLUE)
        draw_text(screen, 'Pause',70,WIDTH/1.95,HEIGHT/2.47,RED)
...
def game():
    ...
    while True:
        ...
        while running: 
            ...
            mobhits=pygame.sprite.groupcollide(mobs, bullets, False, True, pygame.sprite.collide_mask)
            ...
            for boss in bosses:
                healthbar(screen,WIDTH-575,10,boss.health)
